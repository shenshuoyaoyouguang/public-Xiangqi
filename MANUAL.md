...
+ 点击连线菜单，连线参数，打开连线设置对话框，可以对连线程序进行设置。
    ...
    + 后台思考（命中预测续算）：开启后，引擎走出着法时会按其预测的对手应手进行后台思考，对手实际走子与预测一致则命中续算，可加快引擎下一步出招速度；人机对弈与连线对弈均生效，默认关闭。
...
## 5 棋谱
### 5.1 打开棋谱
+ 点击棋谱菜单中的打开棋谱（或工具栏打开按钮），弹出文件选择框，支持 txq、pgn、xqf 三种格式，选择文件后棋谱会加载到右侧棋谱树中，可浏览、回退、前进、查看变招。
### 5.2 保存棋谱
+ 点击工具栏保存按钮，弹出文件保存框，支持保存为 txq、pgn、xqf 三种格式，选择目录与文件名后即可将当前对局的棋谱（含注释与变招）保存到硬盘。
### 5.3 浏览与变招
+ 棋谱树中双击任意节点可跳转到该局面；在历史节点上走出新着法可形成变招，并支持拖拽调整变招列表顺序。
### 5.4 复制文本棋谱
+ 支持复制、粘贴文本棋谱（着法列表），方便在第三方平台与本软件之间交换棋谱内容。
## 6 界面设置
### 6.1 棋步提示
